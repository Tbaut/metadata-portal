...
impl ReactAssetPath {
    pub(crate) fn from_fs_path(path: PathBuf, public_dir: &PathBuf) -> Result<ReactAssetPath> {
...
pub(crate) fn read_export_file(config: &AppConfig) -> Result<IndexMap<ChainName, ExportChainSpec>> {
    let chain_specs =
